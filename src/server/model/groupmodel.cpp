...
vector<Group> GroupModel::queryGroups(int userid) {
    // 先根据userid查询用户所在的组id，再根据组id查询该组的详细信息
    char sql[1024] = {0};
    sprintf(sql, "select a.id, a.groupname, a. groupdesc \
        from allgroup a inner join groupuser b on a.id=b.groupid where b.userid=%d",
        userid);
    ...
    for(Group &group : groupVec) {
        ...
        from user a inner join groupuser b on a.id=b.userid where b.groupid=%d",
        group.getId());
        MYSQL_RES *res = mysql->query(sql);
        if(res != nullptr) {
            MYSQL_ROW row;
